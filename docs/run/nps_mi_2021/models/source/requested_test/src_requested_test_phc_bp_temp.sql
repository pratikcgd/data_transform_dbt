  ...
  create or replace view `nps-omop-project`.`C_SREDH_NPS_MI_STAGING_SIMPLIFIED_2021_VIEWS_ONLY`.`src_requested_test_phc_bp_temp`
  OPTIONS()
  as 
...
CAST(srtb.REQUEST_DATE as string) as REQUEST_DATE,
...
requested_phc_bp_test as (

select q.*, 
SAFE_CAST(hrt1.ENCOUNTER_ID as int) as ENCOUNTER_ID,
SAFE_CAST(srt1.VISIT_DATE as timestamp) as VISIT_DATE 
from requested_phc_bp q 
inner join 
`nps-omop-project`.`B_SREDH_NPS_MI_SF_RDV_BDV_2021`.`LNK_ENCOUNTER_OMOP` lrt1
on q.PATIENT_NUMBER = lrt1.HUB_SITE_PATIENT_SK
inner join 
`nps-omop-project`.`B_SREDH_NPS_MI_SF_RDV_BDV_2021`.`HUB_ENCOUNTER_OMOP` hrt1	 
on lrt1.HUB_ENCOUNTER_SK = hrt1.HUB_ENCOUNTER_SK
inner join 
`nps-omop-project`.`B_SREDH_NPS_MI_SF_RDV_BDV_2021`.`SAT_ENCOUNTER_PHC_BP_OMOP` srt1
on q.REQUEST_DATE = srt1.VISIT_DATE
inner join 
`nps-omop-project`.`B_SREDH_NPS_MI_SF_RDV_BDV_2021`.`LSAT_ENCOUNTER_OMOP` lsrt1
on lrt1.LNK_ENCOUNTER_SK=lsrt1.LNK_ENCOUNTER_SK
where 
	srt1.OMD_CURRENT_RECORD_INDICATOR = 'Y'
	AND srt1.OMD_DELETED_RECORD_INDICATOR = 'N'
	AND lsrt1.OMD_CURRENT_RECORD_INDICATOR = 'Y'
	AND lsrt1.OMD_DELETED_RECORD_INDICATOR = 'N'
)

select * from requested_phc_bp_test;
